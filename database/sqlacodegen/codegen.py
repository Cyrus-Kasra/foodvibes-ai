...
def _render_constraint(constraint):
    ...
    if isinstance(constraint, ForeignKey):
        ...
        remote_columns = [
            "{0}.{1}".format(fk.column.table.fullname, fk.column.name) for fk in constraint.elements
        ]
...
class ModelClass(Model):
    ...
    def __init__(self, table, association_tables, inflect_engine, detect_joined, collector):
        ...
        for association_table in association_tables:
            fk_constraints = [
                c for c in association_table.constraints if isinstance(c, ForeignKeyConstraint)
            ]
...
class ManyToManyRelationship(Relationship):
    def __init__(self, source_cls, target_cls, assocation_table, inflect_engine):
        ...
        constraints = [
            c for c in assocation_table.constraints if isinstance(c, ForeignKeyConstraint)
        ]
        ...
        self.preferred_name = tablename if not colname.endswith("_id") else colname[:-3] + "s"  # type: ignore
        self.backref_name = inflect_engine.plural_noun(self.backref_name)

        # Handle self referential relationships
        if source_cls == target_cls:
            self.preferred_name = "parents" if not colname.endswith("_id") else colname[:-3] + "s"  # type: ignore
...
class CodeGenerator(object):
    ...
    def __init__(
        ...
        for table in metadata.tables.values():
            ...
            fk_constraints = [
                constr for constr in table.constraints if isinstance(constr, ForeignKeyConstraint)
            ]
        ...
        for table in sorted(metadata.tables.values(), key=lambda t: (t.schema or "", t.name)):
            ...
            if notables:
                model = ModelClass(
                    table, links[table.name], inflect_engine, not nojoined, self.collector
                )
