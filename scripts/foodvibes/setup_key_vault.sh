...
typeset slug_local="Key Vault setup"
    ...
    (($rc == 0)) &&
        {
            ...
            (($rc == 0)) && {
                ...
                typeset secret_recs=""
                typeset user_principal_name=$(az ad user list --query "[].{UserPrincipalName:userPrincipalName, Name:displayName}" --output table |
                    grep "^$(echo "${username}" | sed -e 's/@/_/g')#" | cut -d" " -f1)
                ...
                if [[ -z "$user_principal_name" ]]; then
                    ...
                    (($rc == 0)) &&
                        ...
                        secret_recs="$(get_key_vault_secret_all $key_vault_name)" &&
                        logger 1 "Key Vault\n$secret_recs" &&
                        {
                            rc=1
