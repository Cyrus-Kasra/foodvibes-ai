...
class ConfigSingletonClass(object):
    ...
    @classmethod
    def test_credential_retrieve(cls, credential: AzureCliCredential | ManagedIdentityCredential):
        test_client = SecretClient(vault_url=DEFAULT_KEY_VAULT_URL, credential=credential)
    ...
    def acquire_credential(cls) -> AzureCliCredential | ManagedIdentityCredential:
        ...
        try:
            ...
            try:
                logger.info("Unable to get Managed Identity credential. Trying azure CLI")
    ...
    def fetch_key_vault_secrets(cls) -> str | None:
        ...
        cls.maps_api_key = cls.fetch_secret(client=client, secret_name=DEFAULT_BING_MAPS_API_KEY)
        cls.adma_base_url = cls.fetch_secret(client=client, secret_name=DEFAULT_ADMA_BASE_URL)
        cls.adma_client_id = cls.fetch_secret(client=client, secret_name=DEFAULT_ADMA_CLIENT_ID)
        try:
        ...
        cls.adma_authority = cls.fetch_secret(client=client, secret_name=DEFAULT_ADMA_AUTHORITY)
        cls.adma_scope = DEFAULT_ADMA_SCOPE
        cls.adma_default_party = cls.fetch_secret(client=client, secret_name=DEFAULT_ADMA_PARTY_ID)
    ...
    def __init__(self):
        ...
        if len(self.app_insights_instrumentation_key):
            configure_azure_monitor(connection_string=self.app_insights_instrumentation_key)
        self.app = FastAPI(
